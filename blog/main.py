...
@app.post('/user')
def create_user(resquest:schemas.User,db:Session=Depends(get_db)):
    new_user = models.User(id=resquest.id,user_name=resquest.username,email=resquest.email,password=resquest.password)
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
    return new_user
